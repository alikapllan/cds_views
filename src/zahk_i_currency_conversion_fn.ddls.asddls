@AccessControl.authorizationCheck: #NOT_REQUIRED
@EndUserText.label: 'Sales Document Positions with USD Conversion'
@Metadata.allowExtensions: true
@AbapCatalog.viewEnhancementCategory: [ #PROJECTION_LIST ]

define view entity zahk_i_currency_conversion_fn
  with parameters
    @Environment.systemField: #SYSTEM_DATE
    p_calculation_date : abap.dats
  as select from vbap
{
  key vbeln                                                                       as doc_no,
  key posnr                                                                       as pos,
      matnr                                                                       as material_no,

      cast ('EUR' as abap.cuky)                                                   as CurrencyCode,
      @Semantics.amount.currencyCode: 'CurrencyCode' // relating currency to field net_value
      netwr                                                                       as net_value,

      @Semantics.amount.currencyCode: 'CurrencyCode'
      currency_conversion( amount => netwr,
                           source_currency => $projection.CurrencyCode,
                           round => 'X',
                           target_currency => cast( 'USD' as abap.cuky ),
                           exchange_rate_date => $parameters.p_calculation_date ) as net_value_usd,

      @Semantics.quantity.unitOfMeasure: 'weight_unit' // relating unit to brut_weight
      brgew                                                                       as brut_weight,
      @Semantics.quantity.unitOfMeasure: 'weight_unit' // relating unit to net_weight
      ntgew                                                                       as net_weight,
      @EndUserText.quickInfo: 'KG'
      gewei                                                                       as weight_unit,
      ernam                                                                       as created_by
}
where
  ernam <> $session.user;
